...
def detect_feature_dim(root_dir: str, dataset: str) -> int:
    """
    Best-effort detection of feature dimension from the first video.
    Looks up groundTruth files and tries to load corresponding feature file.
    """
    gt_dir = os.path.join(root_dir, dataset, "groundTruth")
    feat_dir = os.path.join(root_dir, dataset, "features")
    if not os.path.isdir(gt_dir) or not os.path.isdir(feat_dir):
        return 0
    try:
        fname = sorted([f for f in os.listdir(gt_dir) if not f.startswith(".")])[0]
    except Exception:
        return 0
    base = os.path.splitext(fname)[0]
    action = parse_action_name(fname, dataset) if "parse_action_name" in globals() else ""
    candidates = [
        os.path.join(feat_dir, action, base + ".npy"),
        os.path.join(feat_dir, action, base + ".txt"),
        os.path.join(feat_dir, base + ".npy"),
        os.path.join(feat_dir, base + ".txt"),
    ]
    for p in candidates:
        if not os.path.isfile(p):
            continue
        try:
            arr = np.loadtxt(p) if p.endswith(".txt") else np.load(p, mmap_mode="r")
...
class VideoSSL(pl.LightningModule):
    ...
    def forward(self, x, mask=None):
        """
        Inference path: run ASOT solver on features and return soft assignments (B x T x K).
        Accepts:
          - x: Tensor shaped (B, T, D) or (B, D, T) or (T, D).
          - mask: Optional BoolTensor (B, T); if None assumes all valid frames.
        """
        if x.dim() == 2:  # T x D
            x = x.unsqueeze(0)
        if x.dim() != 3:
            raise ValueError(f"Expected input with 3 dims (B,T,D) or (B,D,T); got {x.shape}")

        # Detect layout: prefer B x T x D; if middle dim matches feature dim, treat as B x D x T.
        if x.shape[1] == self.layer_sizes[0] and x.shape[2] != self.layer_sizes[0]:
            x = x.transpose(1, 2)
        ...
        with torch.no_grad():
            ...
            feats = F.normalize(self.mlp(x.reshape(-1, D_in)).reshape(B, T, -1), dim=-1)
            temp_prior = asot.temporal_prior(T, self.n_clusters, self.rho, feats.device)
            cost_matrix = 1. - feats @ self.clusters.T.unsqueeze(0)
            cost_matrix += temp_prior
            soft_assign, _ = asot.segment_asot(
                cost_matrix,
                mask,
                eps=self.eval_eps,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p','--base_path', type=str, required=True,
                    help='Root path that contains <dataset>/{groundTruth,features,mapping.txt}')
    # parser.add_argument('--base-path', '-p', type=str, default='/home/users/u6567085/data', help='base directory for dataset')
    parser.add_argument('--dataset', '-d', type=str, required=True, help='dataset to use for training/eval (Breakfast, YTI, FSeval, FS, desktop_assembly, custom)')
    parser.add_argument('--activity', '-ac', type=str, nargs='+', required=False, default=['all'],
                        help='activity classes to select; default=all (no filtering)')
    ...
    parser.add_argument('--ckpt-every', type=int, default=5, help='save a checkpoint every N epochs (early snapshots)')
    parser.add_argument('--train-split', type=str, default='', help='Optional split file for train dataset')
    parser.add_argument('--val-split', type=str, default='', help='Optional split file for val dataset')
    parser.add_argument('--test-split', type=str, default='', help='Optional split file for test dataset')
    args = parser.parse_args()

    pl.seed_everything(args.seed)

    # Auto-adjust input layer size to feature dim if user left default sizes.
    if args.layers == [64, 128, 40]:
        fd = detect_feature_dim(args.base_path, args.dataset)
        if fd > 0:
            print(f"[INFO] Detected feature dim = {fd}; overriding layer_sizes to [{fd}, 128, 40]")
            args.layers = [fd, 128, 40]
    else:
        fd = detect_feature_dim(args.base_path, args.dataset)
        if fd > 0 and fd != args.layers[0]:
            print(f"[WARN] Feature dim {fd} != layer_sizes[0] {args.layers[0]}; consider setting --layers {fd} ...")
        
    data_root = os.path.join(args.base_path, args.dataset)
    def _default_split(name: str) -> str:
        cand = os.path.join(data_root, "splits", name)
        return cand if os.path.isfile(cand) else ""

    train_split = args.train_split or _default_split("train.split1.train")
    val_split = args.val_split or _default_split("val.split1.val")
    test_split = args.test_split or _default_split("test.split1.test")

    if train_split:
        print(f"[INFO] Using train split: {train_split}")
    if val_split:
        print(f"[INFO] Using val split: {val_split}")
    if test_split:
        print(f"[INFO] Using test split: {test_split}")

    data_val = VideoDataset(args.base_path, args.dataset, args.n_frames, standardise=args.std_feats, random=False, split=val_split or None, action_class=args.activity)
    data_train = VideoDataset(args.base_path, args.dataset, args.n_frames, standardise=args.std_feats, random=True, split=train_split or None, action_class=args.activity)
    data_test = VideoDataset(args.base_path, args.dataset, None, standardise=args.std_feats, random=False, split=test_split or None, action_class=args.activity)
    val_loader = DataLoader(data_val, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)
    train_loader = DataLoader(data_train, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    test_loader = DataLoader(data_test, batch_size=1, shuffle=False, num_workers=2, pin_memory=True)
    ...
    ckpt_dir = os.path.join("lightning_logs", f"ckpts_{args.group}")
    ...
    ckpt_cb = ModelCheckpoint(
        ...
        save_top_k=-1,
        save_last=True,
        every_n_epochs=max(1, args.ckpt_every),
