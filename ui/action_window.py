...
class ActionWindow(FrameControlMixin, QWidget):
    def __init__(
        ...
        self._psr_trace_info: Dict[str, Any] = {}
        self._psr_trace_episodes: List[Dict[str, Any]] = []
        self._psr_trace_conflicts: List[Dict[str, Any]] = []
        ...
        self.player = VideoPlayer(status_cb=self._set_status, parent=self)
        try:
    ...
    def _write_label_bank_txt(
        ...
        *,
        ...
        source = labels or [lb.name for lb in (self.labels or [])]
        names = [str(x).strip() for x in source if str(x).strip()]
        if not names:
            return None
        try:
    ...
    def _psr_interruptible_episode_enabled(self) -> bool:
        return bool(self._psr_cfg().get("interruptible_episode", True))

    def _psr_episode_resume_gap_seconds(self) -> float:
        return self._cfg_float(
            self._psr_cfg().get("episode_resume_gap_sec", 4.0),
            4.0,
            lo=0.0,
            hi=120.0,
        )

    def _psr_episode_resume_gap_frames(self) -> int:
        fps = float(max(1, self._get_fps()))
        secs = float(self._psr_episode_resume_gap_seconds())
        return max(0, int(round(fps * secs)))
    ...
    def _psr_recompute_cache(self):
        ...
        if not self._psr_components_match_fixed(self.psr_components):
            self._psr_auto_components_from_labels()
        segments = self._psr_collect_segments()
        derive_result = psr_derive_events(
            segments,
            self.psr_components,
            self.psr_rules,
            ignore_labels=[self._psr_asr_asd_invisible_label],
            resume_gap_frames=(
                self._psr_episode_resume_gap_frames()
                if self._psr_interruptible_episode_enabled()
                else 0
            ),
            return_trace=True,
        )
        if isinstance(derive_result, tuple) and len(derive_result) == 2:
            derived_events = list(derive_result[0] or [])
            self._psr_trace_info = (
                dict(derive_result[1]) if isinstance(derive_result[1], dict) else {}
            )
        else:
            derived_events = list(derive_result or [])
            self._psr_trace_info = {}
        self._psr_trace_episodes = list(self._psr_trace_info.get("episodes") or [])
        ...
        _view_start, _view_end = self._psr_view_range()
        self._psr_trace_conflicts = analyze_trace_conflicts(
            self._psr_trace_episodes,
            self._psr_state_seq,
            self.psr_components,
            initial_state=initial_vec,
        ...
        trace_summary = dict(self._psr_trace_info.get("summary") or {})
        self._psr_diag = {
            ...
            "trace": trace_summary,
            "trace_conflicts": int(len(self._psr_trace_conflicts)),
        }
        self._refresh_trace_review_queue()
    ...
    def _psr_trace_episodes_for_span(
        self, start_frame: int, end_frame: int
    ) -> List[Dict[str, Any]]:
        if end_frame < start_frame:
            start_frame, end_frame = end_frame, start_frame
        hits: List[Dict[str, Any]] = []
        for episode in self._psr_trace_episodes or []:
            try:
                ep_start = int(episode.get("start_frame", 0))
                ep_end = int(episode.get("end_frame", ep_start))
            except Exception:
                continue
            if ep_end < start_frame or ep_start > end_frame:
                continue
            hits.append(episode)
        hits.sort(
            key=lambda item: (
                int(item.get("start_frame", 0)),
                int(item.get("anchor_frame", item.get("end_frame", 0))),
            )
        )
        return hits

    def _psr_trace_summary_line(
        self, start_frame: int, end_frame: int
    ) -> Optional[str]:
        if not self._psr_interruptible_episode_enabled():
            return None
        episodes = self._psr_trace_episodes_for_span(start_frame, end_frame)
        if not episodes:
            return None
        deferred = [
            item
            for item in episodes
            if bool(item.get("deferred_effect"))
            or int(item.get("fragment_count", 1)) > 1
        ]
        target = deferred[0] if deferred else episodes[0]
        if (
            len(episodes) == 1
            and int(target.get("fragment_count", 1) or 1) <= 1
            and not target.get("interrupted")
            and not target.get("deferred_effect")
        ):
            return None
        label = str(target.get("label") or "operation")
        fragments = max(1, int(target.get("fragment_count", 1) or 1))
        anchor = int(target.get("anchor_frame", target.get("end_frame", 0)))
        if len(episodes) == 1:
            prefix = "Trace"
        else:
            prefix = f"Trace ({len(episodes)} active)"
        detail = f"{label}: {fragments} fragment(s), completion @{anchor}"
        if target.get("interrupted"):
            detail += ", interrupted"
        if target.get("deferred_effect"):
            detail += ", deferred state change"
        return f"{prefix}: {detail}"

    @staticmethod
    def _review_item_identity(item: Dict[str, Any]) -> Tuple[str, str, str, int, int, str]:
        return (
            str(item.get("source") or ""),
            str(item.get("view") or ""),
            str(item.get("component_id") or item.get("store") or ""),
            int(item.get("start", item.get("frame", 0)) or 0),
            int(item.get("end", item.get("start", item.get("frame", 0)) or 0) or 0),
            str(item.get("action") or ""),
        )

    def _psr_trace_conflict_review_items(self) -> List[Dict[str, Any]]:
        if not self._psr_interruptible_episode_enabled():
            return []
        if not self.views or not (0 <= self.active_view_idx < len(self.views)):
            return []
        view_name = self.views[self.active_view_idx].get("name", "view")
            ...
            items.append(
                {
                    ...
                    "end": int(end),
                    "frame": int(start),
                    "old": self._psr_state_label(conflict.get("old_state")),
                    "new": self._psr_state_label(repair.get("new_state")),
                    "editor": "trace",
                    "component": conflict.get("component_name")
                    or self._psr_component_name(component_id),
                    "component_id": component_id,
                    "action": f"trace_{conflict.get('conflict_type') or 'repair'}",
                    "desc": conflict.get("reason") or "",
                    "decision": None,
                    "severity": int(conflict.get("severity", 0) or 0),
                    "trace_conflict_type": conflict.get("conflict_type"),
                    "anchor_frame": conflict.get("anchor_frame"),
                    "episode_start": conflict.get("episode_start"),
                    "episode_end": conflict.get("episode_end"),
    ...
    def _refresh_trace_review_queue(self) -> None:
        existing = list(self.review_items or [])
        trace_items = self._psr_trace_conflict_review_items()
        decision_map = {
            self._review_item_identity(item): item.get("decision")
            for item in existing
            if str(item.get("source") or "") == "trace_auto"
        }
        current_key = None
        if 0 <= int(self.review_idx) < len(existing):
            current_key = self._review_item_identity(existing[int(self.review_idx)])
        manual_items = [
            item
            for item in existing
            if str(item.get("source") or "") != "trace_auto"
        ...
        for item in trace_items:
            key = self._review_item_identity(item)
            if key in decision_map:
                item["decision"] = decision_map.get(key)
        merged = manual_items + trace_items
        merged.sort(
            key=lambda item: (
                1 if item.get("decision") is not None else 0,
                -int(item.get("severity", 0) or 0),
                int(item.get("start", item.get("frame", 0)) or 0),
                str(item.get("component_id") or item.get("store") or ""),
            )
        )
        self.review_items = merged
        if not merged:
            self.review_idx = -1
        elif current_key is not None:
            self.review_idx = next(
                (
                    idx
                    for idx, item in enumerate(merged)
                    if self._review_item_identity(item) == current_key
                ),
                min(max(int(self.review_idx), 0), len(merged) - 1),
            )
        elif self.review_idx < 0:
            self.review_idx = 0
        else:
            self.review_idx = min(int(self.review_idx), len(merged) - 1)
        if getattr(self, "review_panel", None) is not None and self.review_panel.isVisible():
    ...
    def _psr_panel_state_context(self, frame: int) -> Dict[str, Any]:
        ...
        trace_summary = self._psr_trace_summary_line(seg_start, seg_end)
        if trace_summary:
    ...
    def _apply_psr_rules(self):
        ...
        self._set_status(
            ...
            f"{diag.get('trace_conflicts', 0)} trace repairs"
        )
    ...
    def _psr_export_trace_payload(
        ...
        *,
        ...
        if not self._psr_interruptible_episode_enabled():
            return None
        info = trace_info if isinstance(trace_info, dict) else self._psr_trace_info
        ...
        for episode in episodes:
            try:
                ...
                end = int(episode.get("end_frame", start))
                anchor = int(episode.get("anchor_frame", end))
            except Exception:
                continue
            if end < int(view_start) or start > int(view_end):
                continue
            fragments = []
            for fragment in episode.get("fragments") or []:
                if not isinstance(fragment, dict):
                    continue
                try:
                    frag_start = int(fragment.get("start_frame", 0))
                    frag_end = int(fragment.get("end_frame", frag_start))
                except Exception:
                    continue
                fragments.append(
                    {
                        "start_frame": max(0, frag_start - int(view_start)),
                        "end_frame": max(0, frag_end - int(view_start)),
                    }
                )
            out_episodes.append(
                {
                    ...
                    "target_state": self._psr_state_value(
                        episode.get("state"), fallback=0
                    ),
                    "start_frame": max(0, start - int(view_start)),
                    "end_frame": max(0, end - int(view_start)),
                    "completion_frame": max(0, anchor - int(view_start)),
                    "fragment_count": int(
                        episode.get("fragment_count", len(fragments) or 1)
                    ),
                    "interrupted": bool(episode.get("interrupted")),
                    "deferred_effect": bool(episode.get("deferred_effect")),
                    "resume_gaps": [
                        int(gap)
        ...
        out_episodes.sort(
            key=lambda item: (
                int(item.get("start_frame", 0)),
                int(item.get("completion_frame", 0)),
                str(item.get("component_id")),
            )
        )
        return {
            "mode": "completion_anchor",
            "resume_gap_frames": int(
                info.get("resume_gap_frames", self._psr_episode_resume_gap_frames())
                or 0
            ),
            "resume_gap_seconds": float(self._psr_episode_resume_gap_seconds()),
    ...
    def _psr_build_export_payload(
        ...
        meta: Dict[str, Any],
        trace_info: Optional[Dict[str, Any]] = None,
        trace_conflicts: Optional[List[Dict[str, Any]]] = None,
    ) -> Dict[str, Any]:
        ...
        trace_payload = self._psr_export_trace_payload(
            view_start=int(view_start),
            view_end=int(view_end),
            trace_info=trace_info,
        )
        if trace_payload:
            base["procedure_trace"] = trace_payload
        repair_payload = []
        for conflict in trace_conflicts or []:
            repair = conflict.get("repair") or {}
            if str(repair.get("kind") or "") != "state_patch":
                continue
            try:
                repair_start = int(repair.get("start", conflict.get("start", 0)))
                repair_end = int(repair.get("end", conflict.get("end", repair_start)))
            except Exception:
                continue
            if repair_end < repair_start:
                repair_start, repair_end = repair_end, repair_start
            repair_payload.append(
                {
                    "type": str(conflict.get("conflict_type") or "trace_conflict"),
                    "component_id": conflict.get("component_id"),
                    "component_name": conflict.get("component_name"),
                    "label": conflict.get("label"),
                    "anchor_frame": max(
                        0, int(conflict.get("anchor_frame", repair_start)) - int(view_start)
                    ),
                    "start_frame": max(0, int(repair_start) - int(view_start)),
                    "end_frame": max(0, int(repair_end) - int(view_start)),
                    "old_state": self._psr_state_value(conflict.get("old_state"), fallback=0),
                    "new_state": self._psr_state_value(repair.get("new_state"), fallback=0),
        ...
        if repair_payload:
            ...
            base["repair_candidates"] = repair_payload
        return base
    ...
    def _psr_batch_convert_dataset(self):
        ...
        for path, info in parsed:
            ...
            if isinstance(derive_result, tuple) and len(derive_result) == 2:
                events = list(derive_result[0] or [])
                trace_info = (
                    dict(derive_result[1]) if isinstance(derive_result[1], dict) else {}
                )
            else:
                events = list(derive_result or [])
                trace_info = {}
    ...
    def _export_psr_asr_asd(self, kind: str):
        ...
        payload = self._psr_build_export_payload(
            ...
            self.psr_components,
            meta,
            trace_info=self._psr_trace_info,
            trace_conflicts=self._psr_trace_conflicts,
        )
        suffix = "asr"
        export_label = "ASR"
    ...
    def _open_review_panel(self):
        ...
        if not self.review_items:
            self._set_review_queue_ui(
                info="Import a review log or enable PSR trace analysis to load review items.",
                progress="0 / 0",
                can_decide=False,
            )
            self._set_status(
                "Review panel opened. Import a review log or inspect PSR trace repairs."
            )
            return
        self._update_review_panel()
    ...
    def _update_review_panel(self):
        ...
        if item.get("kind") == "psr":
            ...
            info_lines = [
                f"Frames: {span_txt}",
                "Track: PSR",
                f"Source: {'Trace repair' if item.get('source') == 'trace_auto' else 'Review log'}",
                f"Component: {comp}",
            ...
            if item.get("source") == "trace_auto":
                if item.get("label"):
                    info_lines.append(f"Episode label: {item.get('label')}")
                if item.get("anchor_frame") is not None:
                    info_lines.append(f"Completion anchor: {item.get('anchor_frame')}")
                if item.get("severity") is not None:
                    info_lines.append(f"Priority: {item.get('severity')}")
        else:
            info_lines = [
                f"Frames: {span_txt}",
