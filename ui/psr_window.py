...
class PSRWindow(QWidget):
    ...
    def update_component_states(
        ...
        if diagnostics:
            ...
            trace_conflicts = int(diagnostics.get("trace_conflicts", 0) or 0)
            if isinstance(diagnostics.get("trace"), dict):
                trace_summary = dict(diagnostics.get("trace") or {})
        init_txt = ""
        try:
        ...
        trace_meta = ""
        trace_deferred = int(trace_summary.get("deferred_count", 0) or 0)
        trace_interrupted = int(trace_summary.get("interrupted_count", 0) or 0)
        if trace_deferred > 0:
            trace_meta = f" | Deferred episodes: {trace_deferred}"
            if trace_interrupted > trace_deferred:
                trace_meta += f" | Interrupted: {trace_interrupted}"
        elif trace_interrupted > 0:
            trace_meta = f" | Interrupted episodes: {trace_interrupted}"
        self.lbl_meta.setText(
            f"Components: {len(self._all_components)}{src} | Rules: {rules_count} | Transitions: {events}{trace_meta}{init_meta}"
        )
        if unmapped or mismatch or trace_conflicts:
            parts = []
            if unmapped:
                parts.append(f"Unmapped labels: {unmapped}")
            if mismatch:
                parts.append(f"Rule mismatch: {mismatch}")
            if trace_conflicts:
                parts.append(f"Trace repairs: {trace_conflicts}")
