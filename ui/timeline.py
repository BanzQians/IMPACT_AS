...
class BaseTimelineRow(QWidget):
    ...
    def _draw_time_grid(self, p: QPainter, start: int, end: int, fps: float):
        g = self.get_gutter()
        span = self.get_span()
        fps_value = max(1.0, float(fps or 0.0))
        # Grid drawing uses frame indices, so quantize non-integer FPS such as 29.97.
        fps_step = max(1, int(round(fps_value)))
        ...
        s_minor = max(1, int(round(fps_value * 0.1)))
        ...
        s_mid = max(1, int(round(fps_value * 0.5)))
        if step_px(s_mid) >= 6:
        ...
        for f in range(first, end + s_major, s_major):
            ...
            if step_px(s_major) >= 60:
                sec = int(round(float(f) / fps_value))
