...
class VideoPlayer(QLabel):
    ...
    def _ensure_audio_player(self):
        ...
        try:
            player.error.connect(self._on_audio_error)
        except Exception:
            pass
    ...
    def load(self, path: str) -> bool:
        ...
        self.frame_rate = fps if fps > 0 else 30.0
        self.crop_start = 0
        ...
        fps_text = f"{self.frame_rate:.3f}".rstrip("0").rstrip(".")
        self._status(f"Loaded: {self.frame_count} frames @ {fps_text} FPS")
    ...
    def pause(self):
        if self.cap and self.is_playing:
            self.is_playing = False
            self.timer.stop()
            if self._audio_enabled and self.audio_player is not None:
    ...
    def attach_audio_from_video(self, media_path: str) -> bool:
        ...
        try:
            ...
            player.setMedia(QMediaContent(QUrl.fromLocalFile(media_path)))
            try:
                if hasattr(player, "setPlaybackRate"):
